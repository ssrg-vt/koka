...
genConstructor info dataRepr (con,conRepr) =
  case conRepr of
    ConEnum{}
       -> return ()
    ConSingleton{conTypeName=typeName}
    ...
    ConSingle{conTypeName=typeName}
       -> -- merge it into the type class itself
          do ctx <- getModule
    ...
    ConStruct{conTypeName=typeName}
       -> conStruct typeName

    ConIso{conTypeName=typeName}
       -> conStruct typeName
...
ppConConstructorEx ctx con conRepr conParams defaults
  = if (null conParams && not (isConNormal conRepr))
     then []
           ...
           (case conRepr of
              ConNormal{conTypeName=typeName}  -> text ":" <+> text "base" <.> parens (ppTag ctx typeName (conInfoName con)) <.> space
              _                      -> empty) <.>
           block (linebreak <.> vcat (
              (case conRepr of
                 ConStruct{conTypeName=typeName} -> [text "this." <.> ppTagName <+> text "=" <+> ppTag ctx typeName (conInfoName con) <.> semi]
                 _             -> [])
...
genDef :: Bool -> Def -> Asm ()
genDef isRec def@(Def name tp expr vis seca attr defsort inl nameRng doc)
  = if (defIsVal def)
            ...
            _ -> let (n,m) = getTypeArities tp
                 in if (m <= 0) then genDefX isRec def else
                     do expr1 <- tetaExpand expr [] n
                        ...
                        genDefX isRec (Def name tp expr2 vis seca attr defsort inl nameRng doc)

genDefZ :: Bool -> Def -> Asm ()
genDefZ isRec def  = genDefX isRec def

genDefX :: Bool -> Def -> Asm ()
genDefX isRec (Def name tp expr vis seca attr isVal inl nameRng doc)
         ...
         do argDocs <- genArguments args
            ...
            result $ hang 2 $ -- cast $
             case repr of
              ...
              ConStruct{conTypeName=typeName} | null args
                -> ppConSingleton ctx typeName tname targs
              ConStruct{conTypeName=typeName}
                -> text "new" <+>
              ...
              _ -> text "new" <+>
                   (case repr of
                      ConAsCons{conTypeName=typeName}
                         -> ppQName ctx (typeClassName typeName)
                      ConSingle{conTypeName=typeName}
                         -> ppQName ctx (typeClassName typeName)
...
genLetDefs isRec defs groups expr
  ...
  where
    uniquefyTopLevel def@(Def name tp expr vis seca attr isVal inl nameRng doc)
      = if not (liftDefToTopLevel  def)
         then return ([],def)
                 ...
                 let newName = qualify (qualifier defname) newVName -- need to qualify or otherwise its considered local
                     newDef = Def newName tp expr vis seca attr isVal inl nameRng ""
                     (m,n)  = getArity tp
...
genLetDef isRec def@(Def name tp expr vis seca attr isVal inl nameRng doc)
...
ppTypeCon ctx c kind
   = let name = typeConName c
     in if (name == nameTpInt)
         ...
         then text "double"
        else if (name == nameTpFloat32) || (name == nameTpFloat16)
         then text "float"

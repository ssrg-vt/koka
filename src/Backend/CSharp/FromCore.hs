...
genDef :: Bool -> Def -> Asm ()
genDef isRec def@(Def name tp expr vis seca defsort inl nameRng doc)
  = if (defIsVal def)
            ...
            _ -> let (n,m) = getTypeArities tp
                 in if (m <= 0) then genDefX isRec def else
                     do expr1 <- tetaExpand expr [] n
                        ...
                        genDefX isRec (Def name tp expr2 vis seca defsort inl nameRng doc)

genDefZ :: Bool -> Def -> Asm ()
genDefZ isRec def  = genDefX isRec def

genDefX :: Bool -> Def -> Asm ()
genDefX isRec (Def name tp expr vis seca isVal inl nameRng doc)
...
genLetDefs isRec defs groups expr
  ...
  where
    uniquefyTopLevel def@(Def name tp expr vis seca isVal inl nameRng doc)
      = if not (liftDefToTopLevel  def)
         then return ([],def)
                 ...
                 let newName = qualify (qualifier defname) newVName -- need to qualify or otherwise its considered local
                     newDef = Def newName tp expr vis seca isVal inl nameRng ""
                     (m,n)  = getArity tp
...
genLetDef isRec def@(Def name tp expr vis seca isVal inl nameRng doc)

...
bindingsTypeDefs modName typeDefGroups
  ...
  where
    ...
    isDefinition td
      = case td of
          DataType binder args cons range vis sort ddef deff isExtend doc seca -> not isExtend
          _ -> True
...
dependencyTypeDef modName typeDef
  = case typeDef of
      ...
      DataType binder args cons range vis sort ddef deff isExtend doc seca -> (typeDefName typeDef, S.map normalize (freeTypes cons))
  where
...
instance (HasFreeTypes t) => HasFreeTypes (UserCon t u k) where
  freeTypes (UserCon name exist params result nameRng rng vis doc seca)

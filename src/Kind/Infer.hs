...
synCopyCon modName info con
  = let rc = rangeHide (conInfoRange con)
        ...
        def  = DefNonRec (Def (ValueBinder defName () (Ann expr fullTp rc) rc rc) rc (dataInfoVis info) (defFun []) InlineAuto "" Nothing)
    in def
  ...
  = let paramss = map (\conInfo -> zipWith (\(name,tp) (pvis,rng) -> (name,(tp,rng,pvis,conInfo)))
        ...
        synAccessor (name,(tp,xrng,visibility,cinfo))
          = let rng      = rangeHide xrng
            ...
            in DefNonRec (Def (ValueBinder defName () expr xrng xrng) rng visibility (defFunEx [Borrow] noFip) InlineAlways doc Nothing)
...
synTester info con
  = let name = (prepend "is-" (toVarName (unqualify (conInfoName con))))
    ...
    in [DefNonRec (Def (ValueBinder name () expr rc rc) rc (conInfoVis con) (defFunEx [Borrow] (Fip (AllocAtMost 0))) InlineAlways doc Nothing)]
...
synConstrTag (con)
  = let name = toOpenTagName (unqualify (conInfoName con))
    ...
    in DefNonRec (Def (ValueBinder name () expr rc rc) rc (conInfoVis con) DefVal InlineNever "" Nothing)
...
infDef (Def binder rng vis isVal inl doc seca)
  = do binder' <- infValueBinder binder
       return (Def binder' rng vis isVal inl doc seca)

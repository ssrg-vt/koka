...
module Syntax.GenDoc( genDoc ) where
...
genDoc env kgamma gamma core p
  ...
  = -- trace ("genIndex: " ++ show (coreProgName core, map showImport (coreProgImports core))) $
    htmlBody $
    do writeLn p $ ptag "h1" "module" (fmtModuleName (coreProgName core) ++
  ...
  where
    ...
    externals
      = concat (map toDef (coreProgExternals core))
      where
        toDef (ExternalImport {}) = []
        toDef ext = [Def (externalName ext) (externalType ext) exprUnit
                         (externalVis ext) Nothing Nothing (defFun (externalParams ext)) InlineAuto (externalRange ext) (externalDoc ext)]
...
fmtImport env kgamma gamma root name (imp)
  = doctag "tr" "" $
    (doctag "td" "code" (indent (length root) $ fmtModName env name (importName imp)) ++
     doctag "td" "" (synopsis env kgamma gamma (importModDoc imp)))
  where
    fmtModName env name qname
    ...
    concat
      [doctag "div" "header code"$
        doctag "span" "def" $
          ...
          ++ fmtImportModuleName env qname -- (atag (linkFromModName env qname "") $ span "module" $ show qname)
...
fmtPrivateImport env kgamma gamma imp
  = doctag "span" "code" $ fmtImportModuleName env (importName imp)

fmtImportModuleName env qname
  = signature env True True "module" qname (qualify qname nameNil) (span "module" (fmtModuleName qname)) $ cspan "namespace" $ fmtModuleName (qname)

...
prettyDefX env isRec def@(Def name scheme expr vis seca attr sort inl nameRng doc)
  = prettyComment env doc $
...
--
prettyExpr env (Let ([DefNonRec (Def x tp e vis seca attr isVal inl nameRng doc)]) e')
...
instance HasTypeVar Def where
  sub `substitute` (Def name scheme expr vis isVal inl nameRng doc seca attr)
    = Def name (sub `substitute` scheme) (sub `substitute` expr) vis isVal inl nameRng doc seca attr

  ftv (Def name scheme expr vis isVal inl nameRng doc seca attr)
    = ftv scheme `tvsUnion` ftv expr

  btv (Def name scheme expr vis isVal inl nameRng doc seca attr)
    = btv scheme `tvsUnion` btv expr

  ftc (Def name scheme expr vis isVal inl nameRng doc seca attr)

...
topDown expr@(App (Lam pars eff body) args) | length pars == length args
  ...
  where
    makeDef (TName npar nparTp) arg
      = DefNonRec (Def npar nparTp arg Private Nothing Nothing DefVal InlineAuto rangeNull "")
...
instance Simplify Def where
  simplify def@(Def name tp expr vis sort inl nameRng doc seca attr)
    = -- trace ("simplifying " ++ show name ) $ -- ++ ": " ++ show expr) $
      do expr' <- inDef name $
                  case expr of
                    ...
                    _ -> simplify expr
         return $ Def name tp expr' vis sort inl nameRng doc seca attr
